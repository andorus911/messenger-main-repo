...
public:
	int sock;
	char client[35];
	//char addr[15]; //сделать конструктор для выделения памяти
	//сдделать метод гет для доступа к сокету
...
int nclients = 0;
lfmp::CSocket clients[0x100]; //отправить в качестве поля в CClient
int clientInd = 0;
...
int main(int argc, char* argv[])
{
	...
	int client_addr_size = sizeof(client_socket.sizeofaddrh());//client_addr); // может взять размер прямо от класса аддр из класса сокета?
...
unsigned long __stdcall PMWorking(void * client_socket)
{
	...
	//переопределять метод ТредМетод для работы с сообщениями - отправка или распечатка
	...
	char buff[20 * 1024];
	char * cbuff = new char[1024];//везде ввести динаммическую память!!!!!!!!!!!!!1111111111111111111
	int k, j;
	int bytes_recv;
	...
	while((bytes_recv = my_sock.recv_h(&buff[0], sizeof(buff), 0)) && bytes_recv != SOCKET_ERROR)//recv(my_sock.sokh, &buff[0], sizeof(buff), 0)) && bytes_recv != SOCKET_ERROR)
	{
		...
		for(int i = 0; i < clientInd; i++)
			if(clients[i].getSock() != my_sock.getSock())
				//send(clients[i].sokh, /*&buff[0]*/&cbuff[0], /*bytes_recv*/k-1, 0);//некоторым отсылаются дублированные сообщения
